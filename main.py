...
def fl_finetune(
    ...
    for epoch in tqdm(range(num_communication_rounds)):
        ...
        for client_id in selected_clients_set:
            if full == False:
                if Adalora:
                    ...
                    if stacking or florist or flex:
                        if heter:
                            config = LoraConfig(
                            r=local_ranks[client_id],
                            lora_alpha=2*local_ranks[client_id],
                        ...
                        if flex and epoch>0:
                            out_path = os.path.join(output_dir, str(epoch-1), 'flex', str(client_id))
                            ...
                            print(f"Client{client_id} weights successfully loaded into the model for training.")
        ...
        if full == False:
            if florist:
                new_weights = torch.load(os.path.join(output_dir, str(epoch), 'florist', 'adapter_model.bin'))
                for name, param in model_client.named_parameters():
                    key = name.replace(".default", "")
                    param_device = param.device
                    print(key)
                ...
                new_weights = torch.load(os.path.join(output_dir, str(epoch), 'fedit', 'adapter_model.bin'))
    ...
    if flex:
        ...
        for i in selected_clients_set:
            ...
            client_acc.append(global_evaluation(model, tokenizer, prompter, dev_data_path))
